...
- **דחיפה ישירה ל-`main`** מאושרת (החלטה של המשתמש ב-2026-05-18). Cloudflare Pages פורס את `main` אוטומטית, אז כל push = deploy תוך כדקה.
- לפני כל push: `npm run build` חייב לעבור. אם הבנייה נכשלה — לתקן לפני שדוחפים.
- ענפי פיתוח (`claude/...`) נשארים זמינים לעבודה גדולה שדורשת ביקורת לפני מיזוג. למיזוג ל-main דרך PR יש להמתין לאישור מפורש של המשתמש.
